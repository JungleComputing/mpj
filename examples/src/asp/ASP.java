...
public class ASP {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (j == i)
                    ...
                    tab[i][j] = 1 + rnd.nextInt(MAX_DISTANCE);
            }
        }
    }
    ...
    static private void iterRange(int lowestValue, int highestValue,
            int numProcs, int rank, int[] startIter, int[] stopIter) {
    ...
    private static void do_asp(int[][] tab, int n, int rank, int size,
            int myStart, int myStop) {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            try {
                MPJ.COMM_WORLD.reduce(tab[i], 0, resTab[i], 0, n, MPJ.INT,
                        MPJ.MIN, 0);
            } catch (MPJException e) {
                System.err.println(e.getMessage());
                e.printStackTrace();
