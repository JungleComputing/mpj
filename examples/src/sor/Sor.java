...
public class Sor {
    ...
    static private void get_bounds(int[] lb, int[] ub, int n, int size, int rank) {
        ...
        if (lb[0] == 0)
            lb[0] = 1; /* row 0 is static */
    }

    /* Processor 0 allocates complete table, others only their part. */
    /*
     * void alloc_grid(double*** Gptr, int nrow, int ncol, int lb, int ub, int
     * rank) { int i; double** G = (double**) malloc(nrow*sizeof(double*)); if (
     * G == 0 ) { fprintf(stderr, "malloc failed\n"); exit(42); }
     * 
     * if(rank == 0) { for (i = 0; i<nrow; i++) { // malloc the own range plus
     * one more line // of overlap on each border G[i] = (double*)
     * malloc(ncol*sizeof(double)); if ( G[i] == 0 ) { fprintf(stderr, "malloc
     * failed\n"); exit(42); } } } else { for (i = lb-1; i<=ub; i++) { //
     * malloc the own range plus one more line // of overlap on each border G[i] =
     * (double*) malloc(ncol*sizeof(double)); if ( G[i] == 0 ) { fprintf(stderr,
     * "malloc failed\n"); exit(42); } } }
     * 
     * Gptr = G; }
     */
     ...
     * int i;
     * 
     * if(rank == 0) { for (i = 0; i<nrow; i++) { free(G[i]); } } else { for (i =
     * lb-1; i<=ub; i++) { // malloc the own range plus one more line
     * free(G[i]); } }
     * 
     * free(G); }
     */
    ...
    static public void main(String[] args) throws MPJException {
        ...
        if (size > 1) {
            if (rank == 0) {
                ...
                for (i = lb[0] - 1; i <= ub[0]; i++) {
                    MPJ.COMM_WORLD.recv(G[i], 0, ncol, MPJ.DOUBLE, 0, i);
                }
            }
        }
        ...
        do {
            ...
            for (phase = 0; phase < 2; phase++) {
                /***************************************************************
                 * this, and ( (send rec send rec) / (rec send rec send) ) are
                 * the *only correct versions. both sides sending at the same
                 * time can lead to deadlock.
                 */
                if (even(rank) == 0) {
                    MPJ.COMM_WORLD
                            .send(G[lb[0]], 0, ncol, MPJ.DOUBLE, pred, 42);
                    MPJ.COMM_WORLD.send(G[ub[0] - 1], 0, ncol, MPJ.DOUBLE,
                            succ, 43);
                    MPJ.COMM_WORLD
                            .recv(G[ub[0]], 0, ncol, MPJ.DOUBLE, succ, 44);
                    MPJ.COMM_WORLD.recv(G[lb[0] - 1], 0, ncol, MPJ.DOUBLE,
                            pred, 45);
                } else {
                    MPJ.COMM_WORLD
                            .recv(G[ub[0]], 0, ncol, MPJ.DOUBLE, succ, 42);
                    MPJ.COMM_WORLD.recv(G[lb[0] - 1], 0, ncol, MPJ.DOUBLE,
                            pred, 43);
                    MPJ.COMM_WORLD
                            .send(G[lb[0]], 0, ncol, MPJ.DOUBLE, pred, 44);
                    MPJ.COMM_WORLD.send(G[ub[0] - 1], 0, ncol, MPJ.DOUBLE,
                            succ, 45);
                }
            ...
            diff[0] = maxdiff[0];
            MPJ.COMM_WORLD.allreduce(diff, 0, maxdiff, 0, 1, MPJ.DOUBLE,
                    MPJ.MAX);
            ...
            if (rank == 0) {
                for (i = 1; i < size; i++) {
                    ...
                    for (j = remoteLb[0]; j < remoteUb[0]; j++) {
                        MPJ.COMM_WORLD.recv(G[j], 0, ncol, MPJ.DOUBLE, i, j);
        ...
        if (rank == 0) {
            ...
            System.out.println("Used " + iteration + " iterations, diff is "
                    + ft.format(maxdiff[0]) + ", allowed diff is "
                    + ft.format(stopdiff));
