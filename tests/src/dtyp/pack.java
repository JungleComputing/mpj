...
class pack {
    static public void test() throws MPJException {
        ...
        if (myrank == src) {
            ...
            status = MPJ.COMM_WORLD.recv(buffer, 0, size[0], MPJ.PACKED, src,
                    99);
            ...
            if (n[0] != 10) {
                errcnt++;
                System.out.println("Wrong value for n; got " + n[0]
                        + " expected 10");
            }
            if (a[0] != 1.1) {
                errcnt++;
                System.out.println("Wrong value for a; got " + a[0]
                        + " expected 1.1");
            }
            if (b[0] != 2.2) {
                errcnt++;
                System.out.println("Wrong value for b; got " + b[0]
                        + " expected 2.2");
            }
        }
