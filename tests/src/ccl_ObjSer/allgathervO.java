...
class allgathervO {
    static public void test() throws MPJException {
        ...
        if (myself == 0)
            MPJ.COMM_WORLD.allgatherv(out, 0, 10, MPJ.OBJECT, in, 0, rcount,
                    dis, MPJ.OBJECT);
        else
            MPJ.COMM_WORLD.allgatherv(out, 0, 5, MPJ.OBJECT, in, 0, rcount,
                    dis, MPJ.OBJECT);
            ...
            if (myself == j) {
                for (k = 0; k < MAXLEN; k++)
                    for (i = 0; i < tasks * stride; i++)
                        if (ans[i] != in[i][k])
                            System.out.println("recived data : " + in[i][k]
                                    + "at [" + i + "][" + k + "] should be : "
                                    + ans[i] + " on proc. : " + j);
            }
