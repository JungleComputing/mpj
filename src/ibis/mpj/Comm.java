...
public class Comm {
    ...
    /**
     * Compare two communicators. MPJ.IDENT results if this communicator and
     * comm1 are references to the same object. MPJ.CONGRUENT results if the
     * underlying groups are identical but the communicators differ by context.
     * MPJ.SIMILAR results if the group members of both communicators are the
     * same but the rank order differs. MPJ.UNEQUAL results otherwise
     * 
     * @param comm1
     *            communicator to compare with
    ...
    public static int compare(Comm comm1, Comm comm2) throws MPJException {
        ...
        if (comm1 == comm2) {
            ...
            if ((comm1.contextId == MPJ.UNDEFINED)
                    || (comm2.contextId == MPJ.UNDEFINED)) {
    ...
    /**
     * Duplicate this communicator. The new communicator is "congruent" to the
     * old one, but has a differnt context.
     * 
    ...
    public Object clone() {
        ...
        try {
            if (!testInter()) {
                ...
                cpComm.group.table = new Vector<IbisIdentifier>(
                        this.group.table);
                return (cpComm);
    ...
    /**
     * <strong> NOT IMPLEMENTED YET. </strong> Create an inter communicator.
     * 
     * @param localComm
     *            local intra communicator
     * @param localLeader
     *            rank of local leader in localComm
     * @param remoteLeader
     *            rank of remote leader in this communicator
     * @param tag
     *            "safe tag"
    ...
    /**
     * Retrieves attribute value by key. Valid key values are MPJ.TAG_UB,
     * MPJ.HOST, MPJ.IO and MPJ.WTIME_IS_GLOBAL
     * 
     * @param keyval
     *            one of the key values predefined by MPJ
    ...
    /**
     * Blocking send operation. The data part of the message consists of a
     * sequence of count values, each of the type indicated by datatype. The
     * actual argument associated with buf must be an array. The value of offset
     * is a subscript in this array, defining the position of the first item of
     * the message. The elements of buf may have primitive type or class type.
    ...
    /**
     * Blocking receive operation. The actual argument associated with buf must
     * be an array. The value offset is a subscript in this array, defining the
     * position into which the first item of the incoming message will be
     * copied.
     * 
     * @param buf
    ...
    public void bsend(Object buf, int offset, int count, Datatype datatype,
            int dest, int tag) throws MPJException {
        IbisMPJComm ibisMPJ = new IbisMPJComm(this, this.rank(), buf, offset,
                count, datatype, this.rank(), dest, tag, IbisMPJComm.OP_IBSEND);
        ibisMPJ.doIbsend();
    }
    ...
    public void ssend(Object buf, int offset, int count, Datatype datatype,
            int dest, int tag) throws MPJException {
        IbisMPJComm ibisMPJ = new IbisMPJComm(this, this.rank(), buf, offset,
                count, datatype, this.rank(), dest, tag, IbisMPJComm.OP_ISSEND);
    ...
    public void rsend(Object buf, int offset, int count, Datatype datatype,
            int dest, int tag) throws MPJException {
        IbisMPJComm ibisMPJ = new IbisMPJComm(this, this.rank(), buf, offset,
                count, datatype, this.rank(), dest, tag, IbisMPJComm.OP_IRSEND);
    ...
    public Request isend(Object buf, int offset, int count, Datatype datatype,
            int dest, int tag) throws MPJException {
        IbisMPJComm ibisMPJComm = new IbisMPJComm(this, this.group.rank(), buf,
                offset, count, datatype, this.group.rank(), dest, tag,
                IbisMPJComm.OP_ISEND);
    ...
    public Request irecv(Object buf, int offset, int count, Datatype datatype,
            int source, int tag) throws MPJException {
        IbisMPJComm ibisMPJComm = new IbisMPJComm(this, this.group.rank(), buf,
                offset, count, datatype, source, this.group.rank(), tag,
                IbisMPJComm.OP_IRECV);
    ...
    /**
     * Check if there is an incoming message matching the pattern specified. If
     * such a message is currently available, a status object similar to the
    ...
    public Status iprobe(int source, int tag) throws MPJException {
        ...
        IbisMPJComm ibisMPJ = new IbisMPJComm(this, this.rank(), null, 0, 0,
                null, source, this.rank(), tag, IbisMPJComm.OP_IPROBE);
        ibisMPJ.doIprobe();
     ...
     * Return a status object similar to the return value of a matching recv
     * operation.
     * 
     * @param source
     *            source rank
    ...
    public Status probe(int source, int tag) throws MPJException {
        ...
        IbisMPJComm ibisMPJ = new IbisMPJComm(this, this.rank(), null, 0, 0,
                null, source, this.rank(), tag, IbisMPJComm.OP_PROBE);
    ...
    /**
     * Creates a persistent communication request for a ready mode send. Further
     * comments as for send.
     * 
     * @param buf
     *            send buffer array
     * @param offset
    ...
    /**
     * Execute a blocking send and receive operation. Further comments as for
     * send and recv.
     * 
     * @param sendbuf
     *            send buffer array
     ...
     *            number of items in receive buffer
     * @param recvtype
     *            datatype of each item in recveive buffer
     * @param source
     *            rank of source
     * @param recvtag
     *            receive tag
    ...
    public Status sendrecv(Object sendbuf, int sendoffset, int sendcount,
            Datatype sendtype, int dest, int sendtag, Object recvbuf,
            int recvoffset, int recvcount, Datatype recvtype, int source,
            int recvtag) throws MPJException {

        Request req = this.irecv(recvbuf, recvoffset, recvcount, recvtype,
                source, recvtag);
    ...
    /**
     * Execute a blocking send and receive operation, receiving message into
     * send buffer. Further comments as for send and recv.
     * 
     * @param buf
     *            buffer array
     * @param offset
     ...
     * @throws MPJException
     */
    public Status sendrecvReplace(Object buf, int offset, int count,
    ...
    /**
     * Packs message in send buffer inbuf into space specified in outbuf. The
     * return value is the output value of position-the initial value
     * incremented by the number of byte written.
     * 
     * @param inbuf
     *            input buffer array
     * @param offset
     ...
     *            output buffer
     * @param position
     *            initial position in output buffer
     ...
     * @throws MPJException
     */
    public byte[] pack(Object inbuf, int offset, int incount, Datatype datatype)
            throws MPJException {
     ...
     * The return value is the output value of position - the internal value
    ...
    /**
     * Returns an upper bound on the increment of position effected by pack. It
     * is an error to call this function if the base type of datatype is
     * MPJ.OBJECT.
     * 
     * @param incount
     *            number of items in input buffer
     ...
     * <strong> NOT IMPLEMENTED YET. </strong>
     * 
     * Returns the type of topology associated with the communicator. The return
     * value will be one of MPJ.GRAPH, MPJ.CART or MPJ.UNDEFINED.
     * 
    ...
    /*
     * public static void errorhandlerSet(Errhandler errhandler) throws
     * MPJException {
     *  }
     * 
        ...
        if (inbuf instanceof byte[]) {
            ...
            if (MPJ.LOCALCOPYIBIS) {
                ...
                ibis.io.DataOutputStream out = new BufferedArrayOutputStream(
                        store_out, 4096);
                ibis.io.DataInputStream in = new BufferedArrayInputStream(
                        store_in, 4096);
                ...
                try {
                    ...
                    mout.writeArray(((Object[]) inbuf), inoffset, count
                            * extent);
                    mout.flush();
    ...
    protected void localcopy2types(Object inbuf, int inoffset, int incount,
            Datatype intype, Object outbuf, int outoffset, int outcount,
            Datatype outtype) throws MPJException {
        ...
        if (inbuf instanceof byte[]) {
            ...
            if (MPJ.LOCALCOPYIBIS) {
                ...
                try {
                    ...
                    mout.writeArray(((Object[]) inbuf), inoffset, incount
                            * inExtent);
                    mout.flush();
                    min.readArray((Object[]) outbuf, outoffset, outcount
                            * outExtent);
