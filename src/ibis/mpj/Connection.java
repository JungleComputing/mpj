...
public class Connection {
    ...
    public Connection(Registry registry, PortType portType, int myRank,
            int commPartnerRank) {
    ...
    protected void setupReceivePort() {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Receive on: " + portString + "; Index: "
                    + commPartnerRank);
        }
        try {
    ...
    protected void setupSendPort() {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Send on: " + portString + "; Index: "
                    + commPartnerRank);
        }
        try {
            sender = MPJ.ibis.createSendPort(portType);
            IbisIdentifier rpHolder = registry.getElectionResult(""
                    + commPartnerRank);
    ...
    protected boolean isConnectionEstablished() {
        return ((sender.connectedTo().length != 0) && (receiver.connectedTo().length != 0));
    }

    protected void putMPJObject(MPJObject obj, Object buf, int offset, int count)
            throws MPJException {
    ...
    protected void receiveHeader(ReadMessage readMsg, int[] header)
            throws MPJException {
        try {
    ...
    protected void receiveData(ReadMessage readMsg, Object buf, int offset,
