...
public class Prequest extends Request {
    ...
    /**
     * Activate a persistent communication request. The communication is
     * completed by using the requests in one of the operations Request.Wait,
     * Request.test, Request.waitAny, Request.testAny, Request.waitAll,
     * Request.testAll, Request.waitSome, Request.testSome. On succesful
     * completion the request becomes inactive. It can be reactivated by a
     * further call to start.
     */
    public void start() {
        try {
            Request req;
            if (mode == OP_SEND) {
                req = comm.isend(this.buf, this.offset, this.count,
                        this.datatype, this.rank, this.tag);
            } else if (mode == OP_BSEND) {
                req = comm.ibsend(this.buf, this.offset, this.count,
                        this.datatype, this.rank, this.tag);
            } else if (mode == OP_SSEND) {
                req = comm.issend(this.buf, this.offset, this.count,
                        this.datatype, this.rank, this.tag);
            } else if (mode == OP_RSEND) {
                req = comm.irsend(this.buf, this.offset, this.count,
                        this.datatype, this.rank, this.tag);
            } else {// if(mode == OP_RECV) {
                req = comm.irecv(this.buf, this.offset, this.count,
                        this.datatype, this.rank, this.tag);
            }
            ...
            for (int i = 0; i < arrayOfRequests.length; i++) {
                ...
                if (arrayOfRequests[i].mode == OP_SEND) {
                    req = arrayOfRequests[i].comm.isend(arrayOfRequests[i].buf,
                            ...
                            arrayOfRequests[i].rank, arrayOfRequests[i].tag);
                } else if (arrayOfRequests[i].mode == OP_BSEND) {
                    req = arrayOfRequests[i].comm.ibsend(
                            arrayOfRequests[i].buf, arrayOfRequests[i].offset,
                            arrayOfRequests[i].count,
                            arrayOfRequests[i].datatype,
                            arrayOfRequests[i].rank, arrayOfRequests[i].tag);
                } else if (arrayOfRequests[i].mode == OP_SSEND) {
                    req = arrayOfRequests[i].comm.issend(
                            arrayOfRequests[i].buf, arrayOfRequests[i].offset,
                            arrayOfRequests[i].count,
                            arrayOfRequests[i].datatype,
                            arrayOfRequests[i].rank, arrayOfRequests[i].tag);
                } else if (arrayOfRequests[i].mode == OP_RSEND) {
                    req = arrayOfRequests[i].comm.irsend(
                            arrayOfRequests[i].buf, arrayOfRequests[i].offset,
                            arrayOfRequests[i].count,
                            arrayOfRequests[i].datatype,
                            arrayOfRequests[i].rank, arrayOfRequests[i].tag);
