...
public class ColAllGatherV {
    ...
    public ColAllGatherV(Object sendbuf, int sendoffset, int sendcount,
            Datatype sendtype, Object recvbuf, int recvoffset, int[] recvcount,
            int[] displs, Datatype recvtype, Intracomm comm, int tag) {
    ...
    protected void call() throws MPJException {
        ...
        for (int i = 1; i < size; i++) {
            /* Status stat = */this.comm.sendrecv(recvbuf, recvoffset
                    + displs[j] * recvtype.extent(), recvcount[j], recvtype,
                    right, this.tag, recvbuf, recvoffset + displs[jnext]
                            * recvtype.extent(), recvcount[jnext], recvtype,
                    left, this.tag);
