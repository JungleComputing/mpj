...
public class ColGatherV {
    ...
    public ColGatherV(Object sendbuf, int sendoffset, int sendcount,
            Datatype sendtype, Object recvbuf, int recvoffset, int[] recvcount,
            int[] displs, Datatype recvtype, int root, Intracomm comm, int tag) {
    ...
    protected void call() throws MPJException {
        ...
        if (rank == root) {
            ...
            for (int i = 0; i < size; i++) {
                if (i != rank)
                    this.comm.recv(recvbuf, this.recvoffset + displs[i]
                            * recvtype.extent(), this.recvcount[i],
                            this.recvtype, i, this.tag);
            }
