...
public class ColAllToAll {
    static Logger logger = LoggerFactory.getLogger(ColAllToAll.class.getName());
    ...
    public ColAllToAll(Object sendbuf, int sendoffset, int sendcount,
    ...
    protected void call() throws MPJException {
        ...
        for (int i = 0; i < this.comm.size(); i++) {
            ...
            request[reqCount] = this.comm.irecv(this.recvbuf, this.recvoffset
                    + (source * recvcount * recvtype.extent()), this.recvcount,
                    this.recvtype, source, this.tag);
        ...
        for (int i = 0; i < size; i++) {
            ...
            request[reqCount] = this.comm.isend(this.sendbuf, this.sendoffset
                    + (dest * sendcount * sendtype.extent()), this.sendcount,
        ...
        if (logger.isDebugEnabled()) {
            for (int i = 0; i < size; i++) {
                logger.debug("status-counts: " + i + ": "
                        + status[i].getCount(recvtype));
            }
        }
    }
}
