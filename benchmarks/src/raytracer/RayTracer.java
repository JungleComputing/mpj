...
public class RayTracer {
    ...
    int datasizes[] = { 150, 500, 2000 };

    long checksum = 0;

    /* create a temporary checksum array for MPJ */
    ...
    public void render(Interval interval) throws MPJException {
        ...
        int p_row[] = new int[((((interval.width * (interval.yto - interval.yfrom)) / interval.width)
                + JGFRayTracerBench.nprocess - 1) / JGFRayTracerBench.nprocess)
                * interval.width];
        ...
        for (y = interval.yfrom + JGFRayTracerBench.rank; y < interval.yto; y += JGFRayTracerBench.nprocess) {
            ...
            for (x = 0; x < interval.width; x++) {
                ...
                // Sets the pixels
                p_row[pixCounter++] = alpha | (red << 16) | (green << 8)
                        | (blue);
            } // end for (x)
        } // end for (y)
        ...
        MPJ.COMM_WORLD.reduce(tmp_checksum, 0, tmp_checksum, 0, 1, MPJ.DOUBLE,
                MPJ.SUM, 0);
        if (JGFRayTracerBench.rank == 0) {
        ...
        if (JGFRayTracerBench.rank == 0) {
            for (int k = 0; k < JGFRayTracerBench.nprocess; k++) {
                if (k != 0) {
                    MPJ.COMM_WORLD.recv(p_row, 0, p_row.length, MPJ.INT, k, k);
                }
                t_count = 0;
            ...
            MPJ.COMM_WORLD.send(p_row, 0, p_row.length, MPJ.INT, 0,
                    JGFRayTracerBench.rank);
        }
