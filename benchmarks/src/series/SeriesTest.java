...
class SeriesTest {
    ...
    void Do() throws MPJException {
        ...
        for (int i = ilow; i < p_array_rows; i++) {
            ...
            p_TestArray[0][i] = TrapezoidIntegrate(
                    (double) 0.0,
                    (double) 2.0,
                    1000,
                    omega
                            * ((double) i + (ref_p_array_rows * JGFSeriesBench.rank)),
                    1); // 1 = cosine term.

            // Calculate the B[i] terms.

            p_TestArray[1][i] = TrapezoidIntegrate(
                    (double) 0.0,
                    (double) 2.0,
                    1000,
                    omega
        ...
        if (JGFSeriesBench.rank == 0) {
            ...
            for (int k = 1; k < JGFSeriesBench.nprocess; k++) {
                ...
                MPJ.COMM_WORLD.recv(p_TestArray[0], 0, p_TestArray[0].length,
                        MPJ.DOUBLE, k, k);
                MPJ.COMM_WORLD.recv(p_TestArray[1], 0, p_TestArray[1].length,
                        MPJ.DOUBLE, k, k + JGFSeriesBench.nprocess);
            ...
            MPJ.COMM_WORLD.ssend(p_TestArray[0], 0, p_TestArray[0].length,
                    MPJ.DOUBLE, 0, JGFSeriesBench.rank);
            MPJ.COMM_WORLD.ssend(p_TestArray[1], 0, p_TestArray[1].length,
                    MPJ.DOUBLE, 0, JGFSeriesBench.rank
                            + JGFSeriesBench.nprocess);
        }
