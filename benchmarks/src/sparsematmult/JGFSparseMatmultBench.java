...
public class JGFSparseMatmultBench extends SparseMatmult implements JGFSection2 {
    ...
    public JGFSparseMatmultBench(int nprocess, int rank) {
        JGFSparseMatmultBench.nprocess = nprocess;
        JGFSparseMatmultBench.rank = rank;
    }
    ...
    public void JGFinitialise() throws MPJException {
        ...
        /*
         * Determine the size of the arrays row,val and col on each process.
         * Note that the array size on process (nprocess-1) may be smaller than
         * the other array sizes.
         */

        p_datasizes_nz = (datasizes_nz[size] + nprocess - 1) / nprocess;
        ref_p_datasizes_nz = p_datasizes_nz;
        rem_p_datasizes_nz = p_datasizes_nz
                - ((p_datasizes_nz * nprocess) - datasizes_nz[size]);
        if (rank == (nprocess - 1)) {
        ...
        /*
         * Initialise the arrays val,col,row. Create full sizes arrays on
         * process 0
         */
        ...
        /*
         * initialise arrays val,col,row on process 0 and send the data to the
         * other processes
         */
        ...
        if (rank == 0) {
            ...
            for (int k = 1; k < nprocess; k++) {
                ...
                for (int i = 0; i < p_datasizes_nz; i++) {
                    buf_row[i + (k * ref_p_datasizes_nz)] = Math.abs(R
                            .nextInt())
                            % datasizes_M[size];
                    buf_col[i + (k * ref_p_datasizes_nz)] = Math.abs(R
                            .nextInt())
                            % datasizes_N[size];
                    buf_val[i + (k * ref_p_datasizes_nz)] = R.nextDouble();
                }
                MPJ.COMM_WORLD.ssend(buf_row, (k * ref_p_datasizes_nz),
                        p_datasizes_nz, MPJ.INT, k, 1);
                MPJ.COMM_WORLD.ssend(buf_col, (k * ref_p_datasizes_nz),
                        p_datasizes_nz, MPJ.INT, k, 2);
                MPJ.COMM_WORLD.ssend(buf_val, (k * ref_p_datasizes_nz),
                        p_datasizes_nz, MPJ.DOUBLE, k, 3);
            }

            p_datasizes_nz = ref_p_datasizes_nz;
        } else {
            MPJ.COMM_WORLD.recv(row, 0, p_datasizes_nz, MPJ.INT, 0, 1);
            MPJ.COMM_WORLD.recv(col, 0, p_datasizes_nz, MPJ.INT, 0, 2);
            MPJ.COMM_WORLD.recv(val, 0, p_datasizes_nz, MPJ.DOUBLE, 0, 3);
        }
    ...
    public void JGFvalidate() {
        ...
        if (rank == 0) {
            ...
            if (dev > 1.0e-12) {
                ...
                System.out.println("ytotal = " + ytotal + "  " + dev + "  "
                        + size);
            }
        }
    ...
    public void JGFrun(int size) throws MPJException {
        ...
        if (rank == 0) {
            JGFInstrumentor.addTimer("Section2:SparseMatmult:Kernel",
                    "Iterations", size);
        }
        ...
        if (rank == 0) {
            JGFInstrumentor.addOpsToTimer("Section2:SparseMatmult:Kernel",
