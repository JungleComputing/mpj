...
void Do() throws MPJException
{
  ...
  if(JGFCryptBench.rank==0) {
    ...
    for(int k=1;k<JGFCryptBench.nprocess;k++){
     ...
     MPJ.COMM_WORLD.ssend(plain1,(p_array_rows*k),m_length,MPJ.BYTE,k,k);
    }
  } else {
    MPJ.COMM_WORLD.recv(p_plain1,0,p_array_rows,MPJ.BYTE,0,JGFCryptBench.rank);
  }
  ...
  if(JGFCryptBench.rank==0) {
    ...
    for(int k=1;k<JGFCryptBench.nprocess;k++) {
      MPJ.COMM_WORLD.recv(plain2,(p_array_rows*k),p_array_rows,MPJ.BYTE,k,k);
    }
  } else {
    MPJ.COMM_WORLD.ssend(p_plain2,0,p_array_rows,MPJ.BYTE,0,JGFCryptBench.rank);
  }
