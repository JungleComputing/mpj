...
public class JGFPingPongBench implements JGFSection1 {
    ...
    public void JGFrun() throws MPJException {
        ...
        for (l = 0; l < MLOOPSIZE; l++) {
            ...
            while (time[0] < TARGETTIME && size < MAXSIZE) {
                if (rank == 0) {
                    ...
                    for (int k = 0; k < size; k++) {
                        MPJ.COMM_WORLD.ssend(arr, 0, arr.length, MPJ.DOUBLE, 1,
                                1);
                        MPJ.COMM_WORLD.recv(arr, 0, arr.length, MPJ.DOUBLE, 1,
                                2);
                    }
                    ...
                    time[0] = JGFInstrumentor
                            .readTimer("Section1:PingPong:Double");
                    JGFInstrumentor.addOpsToTimer("Section1:PingPong:Double",
                            (double) size * arr.length * 8);

                } else {

                    /* Carryout the pingpong on process 1 */
                    for (int k = 0; k < size; k++) {
                        MPJ.COMM_WORLD.recv(arr, 0, arr.length, MPJ.DOUBLE, 0,
                                1);
                        MPJ.COMM_WORLD.ssend(arr, 0, arr.length, MPJ.DOUBLE, 0,
                                2);
                    }
            ...
            MPJ.COMM_WORLD.barrier();
            if (rank == 0) {
                JGFInstrumentor.printperfTimer("Section1:PingPong:Double",
                        arr.length);
            }
        ...
        for (l = 0; l < MLOOPSIZE; l++) {
            ...
            while (time[0] < TARGETTIME && size < MAXSIZE) {
                if (rank == 0) {
                    ...
                    for (int k = 0; k < size; k++) {
                        MPJ.COMM_WORLD.ssend(arr_obj, 0, arr_obj.length,
                                MPJ.OBJECT, 1, 1);
                        MPJ.COMM_WORLD.recv(arr_obj, 0, arr_obj.length,
                                MPJ.OBJECT, 1, 2);
                    }
                    ...
                    time[0] = JGFInstrumentor
                            .readTimer("Section1:PingPong:Object");
                    JGFInstrumentor.addOpsToTimer("Section1:PingPong:Object",
                            (double) size * arr_obj.length);
            ...
            MPJ.COMM_WORLD.barrier();
            if (rank == 0) {
                JGFInstrumentor.printperfTimer("Section1:PingPong:Object",
                        arr_obj.length);
            }
    ...
    public static void main(String[] argv) throws MPJException {
        ...
        if (nprocess != 2) {
            if (rank == 0) {
                System.out
                        .println("The JGFPingPong benchmark may only be executed on 2 processes");
            }
            MPJ.COMM_WORLD.barrier();
            System.exit(0);
        }
