...
public class JGFScatterBench implements JGFSection1 {
    ...
    public JGFScatterBench(int rank, int nprocess) {
        JGFScatterBench.nprocess = nprocess;
        JGFScatterBench.rank = rank;
    }
    ...
    public void JGFrun() throws MPJException {
        ...
        for (l = 0; l < MLOOPSIZE; l++) {
            ...
            while (time[0] < TARGETTIME && size < MAXSIZE) {
                ...
                for (int k = 0; k < size; k++) {
                    MPJ.COMM_WORLD.scatter(send_arr, 0, recv_arr.length,
                            MPJ.DOUBLE, recv_arr, 0, recv_arr.length,
                            MPJ.DOUBLE, 0);
                    MPJ.COMM_WORLD.barrier();
                ...
                if (rank == 0) {
                    ...
                    time[0] = JGFInstrumentor
                            .readTimer("Section1:Scatter:Double");
                    JGFInstrumentor.addOpsToTimer("Section1:Scatter:Double",
                            (double) size * recv_arr.length * 8);
                }
            ...
            if (rank == 0) {
                ...
                JGFInstrumentor.addTimeToTimer("Section1:Scatter:Double",
                        -b_time);
                JGFInstrumentor.printperfTimer("Section1:Scatter:Double",
                        recv_arr.length);
            }
        ...
        for (l = 0; l < MLOOPSIZE; l++) {
            ...
            while (time[0] < TARGETTIME && size < MAXSIZE) {
                ...
                /* Carryout the broadcast operation */
                for (int k = 0; k < size; k++) {
                    MPJ.COMM_WORLD.scatter(send_arr_obj, 0,
                            recv_arr_obj.length, MPJ.OBJECT, recv_arr_obj, 0,
                            recv_arr_obj.length, MPJ.OBJECT, 0);
                    MPJ.COMM_WORLD.barrier();
                ...
                if (rank == 0) {
                    ...
                    time[0] = JGFInstrumentor
                            .readTimer("Section1:Scatter:Object");
                    JGFInstrumentor.addOpsToTimer("Section1:Scatter:Object",
                            (double) size * recv_arr_obj.length);
                }
            ...
            if (rank == 0) {
                JGFInstrumentor.stopTimer("Section1:Scatter:Barrier");
                b_time = JGFInstrumentor.readTimer("Section1:Scatter:Barrier");
                JGFInstrumentor.addTimeToTimer("Section1:Scatter:Object",
                        -b_time);
                JGFInstrumentor.printperfTimer("Section1:Scatter:Object",
                        recv_arr_obj.length);
            }
