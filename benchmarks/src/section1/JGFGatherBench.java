...
public class JGFGatherBench implements JGFSection1 {
    ...
    public JGFGatherBench(int rank, int nprocess) {
        JGFGatherBench.nprocess = nprocess;
        JGFGatherBench.rank = rank;
    }
    ...
    public void JGFrun() throws MPJException {
        ...
        for (l = 0; l < MLOOPSIZE; l++) {
            ...
            while (time[0] < TARGETTIME && size < MAXSIZE) {
                ...
                for (int k = 0; k < size; k++) {
                    // System.err.println("iter: " + k + ", size = " +
                    // send_arr.length);
                    MPJ.COMM_WORLD.gather(send_arr, 0, send_arr.length,
                            MPJ.DOUBLE, recv_arr, 0, send_arr.length,
                            MPJ.DOUBLE, 0);
                    MPJ.COMM_WORLD.barrier();
                ...
                /*
                 * Stop the timer. Note that this reports no of bytes sent per
                 * process
                 */
                if (rank == 0) {
                    JGFInstrumentor.stopTimer("Section1:Gather:Double");
                    time[0] = JGFInstrumentor
                            .readTimer("Section1:Gather:Double");
                    JGFInstrumentor.addOpsToTimer("Section1:Gather:Double",
                            (double) size * send_arr.length * 8);
                }
            ...
            /*
             * determine the cost of the Barrier, subtract the cost and write
             * out the performance time
             */
            MPJ.COMM_WORLD.barrier();
            if (rank == 0) {
            ...
            if (rank == 0) {
                ...
                JGFInstrumentor.addTimeToTimer("Section1:Gather:Double",
                        -b_time);
                JGFInstrumentor.printperfTimer("Section1:Gather:Double",
                        send_arr.length);
            }
        ...
        for (l = 0; l < MLOOPSIZE; l++) {
            ...
            while (time[0] < TARGETTIME && size < MAXSIZE) {
                ...
                /* Carryout the broadcast operation */
                for (int k = 0; k < size; k++) {
                    MPJ.COMM_WORLD.gather(send_arr_obj, 0, send_arr_obj.length,
                            MPJ.OBJECT, recv_arr_obj, 0, send_arr_obj.length,
                            MPJ.OBJECT, 0);
                    MPJ.COMM_WORLD.barrier();
                ...
                if (rank == 0) {
                    ...
                    time[0] = JGFInstrumentor
                            .readTimer("Section1:Gather:Object");
                    JGFInstrumentor.addOpsToTimer("Section1:Gather:Object",
                            (double) size * send_arr_obj.length);
                }
            ...
            if (rank == 0) {
                JGFInstrumentor.stopTimer("Section1:Gather:Barrier");
                b_time = JGFInstrumentor.readTimer("Section1:Gather:Barrier");
                JGFInstrumentor.addTimeToTimer("Section1:Gather:Object",
                        -b_time);
                JGFInstrumentor.printperfTimer("Section1:Gather:Object",
                        send_arr_obj.length);
            }
