...
public class JGFLUFactBench extends Linpack implements JGFSection2 {
    ...
    public JGFLUFactBench(int nprocess, int rank) {
        JGFLUFactBench.nprocess = nprocess;
        JGFLUFactBench.rank = rank;
    }
    ...
    public void JGFinitialise() throws MPJException {
        ...
        /*
         * determine the size of the sub arrays and copy the data in a cyclic
         * distribution to the sub arrays.
         */
        ...
        if (rank == 0) {
            ...
            for (int i = 0; i < a.length; i++) {
                if (r_count == 0) {
                    ...
                    MPJ.COMM_WORLD.send(a, i, 1, MPJ.OBJECT, r_count, 10);
                }
            ...
            for (int i = 0; i < real_p_ldaa; i++) {
                MPJ.COMM_WORLD.recv(buf_a, i, 1, MPJ.OBJECT, 0, 10);
            }
        ...
        MPJ.COMM_WORLD.bcast(list, 0, list.length, MPJ.INT, 0);
        MPJ.COMM_WORLD.bcast(buf_list, 0, list.length, MPJ.INT, 0);
    ...
    public void JGFrun(int size) throws MPJException {
        ...
        if (rank == 0) {
            JGFInstrumentor.addOpsToTimer("Section2:LUFact:Kernel",
                    ops / 1.0e06);
