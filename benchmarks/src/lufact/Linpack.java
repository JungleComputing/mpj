...
public class Linpack {
    ...
    final int dgefa(double a[][], int lda, int n, int ipvt[])
        ...
        /* create temporary array for MPJ */
        int[] tmp_l = new int[1];
        ...
        if (nm1 >= 0) {
            for (k = 0; k < nm1; k++) {
                ...
                // prevent finished threads from indexing an non-existent
                // element
                ...
                MPJ.COMM_WORLD.bcast(buf_col_k, 0, buf_col_k.length,
                        MPJ.DOUBLE, list[k]);
                tmp_l[0] = l;
                MPJ.COMM_WORLD.bcast(tmp_l, 0, tmp_l.length, MPJ.INT, list[k]);
                l = tmp_l[0];
                if (buf_col_k[k] != 0) {
                    ...
                    for (j = kp1; j < n; j++) {
                        if (list[j] == JGFLUFactBench.rank) {
                            ...
                            daxpy(n - (kp1), t, buf_col_k, kp1, 1, col_j, kp1,
                                    1);
                        }
                    }
                } else {
        ...
        if (JGFLUFactBench.rank == 0) {
            ...
            for (int i = 0; i < a.length; i++) {
                if (list[i] == JGFLUFactBench.rank) {
                    ...
                    MPJ.COMM_WORLD.recv(a, i, 1, MPJ.OBJECT, list[i], 10);
                }
            }
        } else {
            for (int i = 0; i < JGFLUFactBench.real_p_ldaa; i++) {
                MPJ.COMM_WORLD.send(buf_a, i, 1, MPJ.OBJECT, 0, 10);
            }
        }
