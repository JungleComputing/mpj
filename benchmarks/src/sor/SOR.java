...
public class SOR {
    ...
    public static final void SORrun(double omega, double p_G[][],
            int num_iterations, double G[][]) throws MPJException {
        ...
        for (int p = 0; p < 2 * num_iterations; p++) {
            for (int i = ilow + (p % 2); i < ihigh; i = i + 2) {
                ...
                if (i != 0) {
                    ...
                    if ((i == 1) && JGFSORBench.rank == 0) {
                            ...
                            && JGFSORBench.rank == (JGFSORBench.nprocess - 1)) {

                    } else if (((i == 2) && JGFSORBench.rank == 0)
                            || ((i == 1) && (JGFSORBench.rank != 0))) {
            ...
            if (JGFSORBench.rank != JGFSORBench.nprocess - 1) {
                MPJ.COMM_WORLD.sendrecv(p_G[p_G.length - 2], 0,
                        p_G[p_G.length - 2].length, MPJ.DOUBLE,
                        JGFSORBench.rank + 1, 1, p_G[p_G.length - 1], 0,
                        p_G[p_G.length - 1].length, MPJ.DOUBLE,
                        JGFSORBench.rank + 1, 2);
            }
            if (JGFSORBench.rank != 0) {
                MPJ.COMM_WORLD.sendrecv(p_G[1], 0, p_G[1].length, MPJ.DOUBLE,
                        JGFSORBench.rank - 1, 2, p_G[0], 0, p_G[0].length,
                        MPJ.DOUBLE, JGFSORBench.rank - 1, 1);
            }
        ...
        if (JGFSORBench.rank == 0) {
            ...
            for (int k = 1; k < JGFSORBench.nprocess; k++) {
                ...
                MPJ.COMM_WORLD.recv(G, k * JGFSORBench.p_row, rm_length,
                        MPJ.OBJECT, k, k);
                System.gc();
            }
            ...
            for (int k = 1; k < JGFSORBench.nprocess; k++) {
                if (JGFSORBench.rank == k) {
                    MPJ.COMM_WORLD.ssend(p_G, 1, JGFSORBench.p_row, MPJ.OBJECT,
                            0, JGFSORBench.rank);
                }
            }
        }
