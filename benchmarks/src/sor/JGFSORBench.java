...
public class JGFSORBench extends SOR implements JGFSection2 {
    ...
    private int datasizes[] = { 1000, 1500, 2000, 6000 };
    ...
    public JGFSORBench(int nprocess, int rank) {
        JGFSORBench.nprocess = nprocess;
        JGFSORBench.rank = rank;
    }
    ...
    public void JGFkernel() throws MPJException {
        ...
        if (rank == 0) {
            ...
            for (int k = 1; k < nprocess; k++) {
                ...
                MPJ.COMM_WORLD.send(G, (k * p_row) - 1, m_length, MPJ.OBJECT,
                        k, k);
            }

        } else {

            MPJ.COMM_WORLD.recv(p_G, 0, p_row + 2, MPJ.OBJECT, 0, rank);
        }
    ...
    public void JGFvalidate() {
        ...
        // commented out, because we don't know the refval of problem size of
        // 6000
        /*
         * // double refval[] =
         * {0.4984199298207158,1.123010681492097,1.9967774998523777}; double
         * refval[] = {0.498574406322512,1.1234778980135105,1.9954895063582696};
         * 
         * if(rank==0) { double dev = Math.abs(Gtotal - refval[size]); if (dev >
         * 1.0e-12 ){ System.out.println("Validation failed");
