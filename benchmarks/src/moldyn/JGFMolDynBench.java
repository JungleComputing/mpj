...
public class JGFMolDynBench extends md implements JGFSection3 {
    ...
    public JGFMolDynBench(int nprocess, int rank) {
        JGFMolDynBench.nprocess = nprocess;
        JGFMolDynBench.rank = rank;
    }
    ...
    public void JGFvalidate() {
        // commented out, because we don't know the ref_value of a problem size
        // of 19
        /*
         * double refval[] = {1731.4306625334357,7397.392307839352}; double dev =
         * Math.abs(ek - refval[size]); if (dev > 1.0e-12 ){
         * System.out.println("Validation failed"); System.out.println("Kinetic
         * Energy = " + ek + " " + dev + " " + size); }
         */
    }
    ...
    public void JGFrun(int size) throws MPJException {
        ...
        if (rank == 0) {
            JGFInstrumentor
                    .addTimer("Section3:MolDyn:Total", "Solutions", size);
            JGFInstrumentor.addTimer("Section3:MolDyn:Run", "Interactions",
                    size);
        }
        ...
        if (rank == 0) {
            ...
            JGFInstrumentor.addOpsToTimer("Section3:MolDyn:Run",
                    (double) interactions);
